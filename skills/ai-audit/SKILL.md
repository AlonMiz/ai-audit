...
**Rules:**
- Read files only. Do NOT modify any project file except writing the report at the end (see Step 6b for path and naming rules).
...
────────────────────────────────────────────────────────────
Report written to: reports/ai-audit-report-<date>.md
```

### 6b — Write the report to `reports/`

Determine the output path:
1. Use today's date in `YYYY-MM-DD` format → try `reports/ai-audit-report-YYYY-MM-DD.md`
2. If that file already exists, append the current time (24-hour, `HHmm`) → `reports/ai-audit-report-YYYY-MM-DD-HHmm.md`
3. Create the `reports/` directory if it does not exist.

Write the file at the resolved path with this exact structure:
...
After writing the file, confirm in chat with the exact path used, e.g.: "Report written to `reports/ai-audit-report-2026-05-16.md`."
